...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: project?.title || "Project", robots: project?.publishState === "published" ? undefined : { index: false, follow: false } };
}

export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const project = projects.find((item) => item.slug === slug && item.publishState === "published");
  if (!project) notFound();
  return (
    <>
      ...
      <Section>
        <Inner className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-lg bg-white p-7 shadow-sm">
            <h2 className="font-serif text-3xl font-bold text-chocolate">Funding progress</h2>
      ...
      <Section className="bg-sand/35">
        <Inner className="grid gap-6 md:grid-cols-3">
          <article className="rounded-lg bg-white p-6 shadow-sm"><h2 className="font-serif text-2xl font-bold text-chocolate">The need</h2><p className="mt-3 leading-7 text-charcoal/78">{project.need}</p></article>
          <article className="rounded-lg bg-forest p-6 text-white shadow-sm"><h2 className="font-serif text-2xl font-bold">Why it matters</h2><p className="mt-3 leading-7 text-white/86">{project.whyItMatters}</p></article>
          <article className="rounded-lg bg-white p-6 shadow-sm"><h2 className="font-serif text-2xl font-bold text-chocolate">Our response</h2><p className="mt-3 leading-7 text-charcoal/78">{project.solution}</p></article>
        </Inner>
      </Section>
      <Section>
          ...
          <div className="rounded-lg bg-white p-7 shadow-sm">
            ...
            <ul className="mt-6 grid gap-4">
              {project.supportMakesPossible.map((item) => (
                <li key={item} className="flex gap-3 text-lg leading-7 text-charcoal/78"><span className="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-gold" /><span>{item}</span></li>
              ))}
            </ul>
          </div>
        </Inner>
      ...
      <Section className="bg-forest text-white">
        <Inner>
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-lg leading-8 text-white/82">Every project should move at the pace of trust. We confirm the need, listen locally, use resources thoughtfully, and share what we can without turning families into displays.</p>
          </div>
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {project.approach.map((item) => <article key={item} className="rounded-lg border border-white/15 bg-white/8 p-5"><p className="leading-7 text-white/84">{item}</p></article>)}
          </div>
        </Inner>
      </Section>
